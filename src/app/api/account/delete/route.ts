// Pure EQ domain — account deletion (right-to-erasure).
//
...
// DELETE CASCADE (verified across migrations 0001+), so this one call erases all
// of the user's data — entries, conversations, persons, profile, coins ledger,
// reflections, archives — in one transaction. If a future table is added WITHOUT
// a cascading FK to auth.users, it would orphan rows here; new user-scoped tables
// must keep that cascade (see CLAUDE.md / migration conventions).
