...
// Stashed pre-auth quiz blobs are only accepted at flush time if the user
// either matches the stamp (authed user re-flushing their own pending) or
// just signed up within this window (handles the pre-auth stash → signup
// flow). Longer than signup-then-confirm-email takes realistically; shorter
// than "a day later, stranger B logs in on this laptop."
const FRESH_SIGNUP_WINDOW_MS = 30 * 60 * 1000;

type PendingOnboarding = {
  // user hint — email or id captured at stash time. Null for pre-auth stashes
  // (user didn't have an account yet). Cross-account guard: non-null hint
  // must equal currentHint at flush, OR the current user must have been
  // created within FRESH_SIGNUP_WINDOW_MS of stashedAt — that means only the
  // user who just signed up from this stash can claim it, not some other
  // established user who happens to log in on the same device later.
...
export default function OnboardingClient() {
  ...
  // Client mounts only when the server gate let us through: either unauthed,
  // or authed without a profile, or ?retake=1. On mount:
  //   1. Pending quiz in sessionStorage + authed (and hint matches) → flush.
  //   2. Otherwise → show quiz.
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        // Flush a pending submission stashed from the pre-auth quiz.
        ...
        if (
          ...
          // Cross-account contamination guard. Two legitimate paths:
          //   (A) authed user re-flushing their own stash → userHint matches
          //       current hint.
          //   (B) pre-auth user who just signed up → userHint is null AND
          //       the current user was created within the fresh-signup
          //       window of the stash timestamp.
          // Anything else (null hint but established user; mismatched hint;
          // impossibly-old stash) is discarded so a later stranger logging in
          // on the same device doesn't inherit the first user's quiz.
  ...
  async function handleCta() {
    ...
    try {
      ...
      if (!user) {
        // Stash with timestamp so the flush-side can bind to a fresh-signup
        // window — prevents a stranger who logs in on this device later from
        // inheriting these answers. See FRESH_SIGNUP_WINDOW_MS above.
