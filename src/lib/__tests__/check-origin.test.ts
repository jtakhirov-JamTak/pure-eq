...
function req(headers: Record<string, string>): Request {
  return new Request("https://pure-eq.vercel.app/api/x", { headers });
}
    ...
    expect(
      checkOrigin(
        req({
          ...
          origin: "https://pure-eq.vercel.app",
          host: "pure-eq.vercel.app",
        }),
      ),
    ).toBe(false);
  });
...
describe("checkOrigin — Origin/Host fallback (Sec-Fetch-Site absent)", () => {
  it("accepts when the Origin host matches the Host header", () => {
    expect(
      checkOrigin(
        req({ origin: "https://pure-eq.vercel.app", host: "pure-eq.vercel.app" }),
      ),
    ).toBe(true);
  });
  ...
  it("rejects when the Origin host differs from the Host header", () => {
    expect(
      checkOrigin(req({ origin: "https://evil.com", host: "pure-eq.vercel.app" })),
    ).toBe(false);
  });

  it("rejects when Origin is missing", () => {
    expect(checkOrigin(req({ host: "pure-eq.vercel.app" }))).toBe(false);
  });

  it("rejects when Host is missing", () => {
    expect(checkOrigin(req({ origin: "https://pure-eq.vercel.app" }))).toBe(false);
  });

  it("rejects a malformed Origin (URL parse throws)", () => {
    expect(
      checkOrigin(req({ origin: "not a url", host: "pure-eq.vercel.app" })),
    ).toBe(false);
  });
});
