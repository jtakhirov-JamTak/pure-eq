...
When Claude makes a mistake, add the lesson to the "Lessons Learned" section below — or to `docs/Engineering_Playbook.txt` if it's reusable beyond Pure EQ (but only after it's been re-used by a second app, not speculatively).
...
- **Engineering Playbook** (`docs/Engineering_Playbook.txt`) — only lessons reusable beyond Pure EQ. Promotion happens when a second app re-uses a rule, not speculatively. Universal patterns (validation, auth, rate limit, idempotency, CSRF, raw+derived, stale closures, timers, fetch res.ok, magic-byte sniff, extraction via confidence gate, etc.) live there in §3–§16 — read the playbook FIRST, then the deltas below.
...
- **AI output `.max()` caps must come from observed output length + margin, not aesthetic intuition — AND the prompt must tell the model how to handle the cap.** The 04-20 Review incident: caps set at 120/150/200 silently rejected concrete behavior-level outputs (268 chars on `how_user_likely_came_across`); both retries hit the same wall and the user saw "Entry saved, Try again" with no feedback. Unified all 3 coach modules to `.max(300)` + bumped `aiVersionValue` 2→3 so tight-cap rows stay distinguishable from loose-cap rows in analytics. The BREVITY block in `prompts.ts` states the HARD limit, gives per-field-type target ranges (phrase 80–150, paragraph 200–280), and — critically — instructs the model on what to do when approaching the cap: cut qualifiers and hedging, keep the concrete cause→effect read, NEVER fall back to a category label to save chars (a short category label is worse than a truncated concrete read). Schema is the backstop; prompt guidance keeps typical outputs well under the cap. Universal pattern in `docs/Engineering_Playbook.txt` §16.13.
...
- **FK cascades from `auth.users` make `admin.deleteUser()` data-safe.** Every user-scoped table uses `on delete cascade`, so test-user cleanup in one call also cleans `user_profiles`, `raw_records`, all derived tables, `persons`, `threads`. Verified across migrations 0001–0013.
...
- **Error-path UX must offer a next action, not a dead end.** On write-succeeds-AI-fails, never land on "Done → back to /coach" — offer "Try again for coaching feedback" (safe because the idempotency key dedupes). And don't write promises into the UI ("this contributes to your insights") that the product doesn't actually keep yet.
- **Don't persist telemetry fields until a consumer reads them.** `inputModes`, `usedVoiceRef` sticky-flag counters — every feature rots or lies without a pipeline or dashboard reading it. Add telemetry when a consumer exists, not speculatively.
- **Freshness labels must reflect actual computation behavior.** "Next update at 12" implies batch processing; v0 recomputes on every page load. Use accurate text ("Based on N entries across M days") until real batch regeneration ships.
...
- **Positive counter-patterns need the same `emergingTagCount` (2+) as negative patterns.** A single observation is noise, not insight. Applies to "How You Tend to Land" and per-person positive patterns.
