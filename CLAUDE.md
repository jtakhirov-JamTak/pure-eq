...
- **Symmetric `generator_version` check on both reader and writer.** `src/lib/insights/generate.ts` exports `GENERATOR_VERSION` (currently `"reflection_v5"`); the page reads it and gates rendering on `row.generator_version === GENERATOR_VERSION`, mismatch falls through to ReflectionKickoff (Playbook §16.17 generalized). Bump the constant whenever the `ai_json` shape changes — every shape edit so far (v3→v4 confidence relabel, v4→v5 focus loop) bumped it, which abandons old rows into a free regenerate rather than rendering them broken.
