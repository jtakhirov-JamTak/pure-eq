...
- **Side effects (thread creation, row inserts) must be inside the idempotency-guarded branch.** Thread auto-create in Prepare was running BEFORE the idempotency check, creating duplicate threads on retry/strict-mode. Any side effect that creates new rows must be gated behind `if (!existingRaw)`.
