...
| **Full gate**     | `npm run verify` — tsc → lint → tests → build       |
...
**The playbook runs to §16.32** — note the heading style, `### 16.x`, normalized 2026-08-19 so a single grep finds all of them. Fifteen bullets were deleted from this section on that date because they restated a section that already existed. If you're looking for a rule that used to be here: AI output length caps → §16.13 · runtime-narrowing enum columns read from DB → §16.14 · empty-state render shape → §16.15 · cache-version guard symmetry → §16.17 · access-anchor RLS pinning → §16.20 · the weekly-reflection pattern end to end (cost gate, loud INSERT failures, "Generated {date}" byline, quote-to-source binding, version-guarded reads) → §16.22 · never CHECK an enum a writer also knows → §16.23 · set-then-submit stale state → §16.7. Two more moved up to the user-global layer: the mobile baseline → `MOBILE-FLOOR` and webhook gate exclusions → `GATE-EXCLUSIONS`, both in `~/.claude/REVIEWER_CONVENTIONS.md` §6.

One rule was deleted as **wrong**, not redundant: "origin check applies to enumeration GETs". Cross-site `fetch()` can't read a response without CORS, `SameSite=Lax` doesn't send cookies on those requests, and same-origin XSS makes the check moot — while `Origin` is absent on same-origin navigations, so it 403s download buttons. Guard reads with cookie `SameSite` + a tight CORS allow-list. See **READ-SURFACE-LIMITS** §6. Don't reintroduce it.
...
- **The single charge primitive is `src/lib/coins.ts`** (`spendCoins`/`refundCoins`/`costForTier`/`getBalance` + `nextGenerationAttempt`/`generationSpendKey`). Reserve-at-start / refund-on-failure, per-attempt spend key. THREE charge orchestrators duplicate the policy — Coach `billed-generation.ts` (extracted from run-module), Insights `generate.ts` (callbacks), and Monthly Report `monthly-report.ts` (same callback shape, B4 2026-06-12) — so a finalize-on-success change must touch ALL THREE (memory `project_coin_charge_dual_shape`). All refund on AI failure, on a derived-write failure, AND on a refusal/downgrade — keep them symmetric. The two Insights ROUTES also duplicate the ~30-line reserve/refund callback verbatim; extracting a shared `makeInsightsCoinCallbacks` helper is the deferred refactor if a fourth consumer appears.
...
- **Unique-index migrations need a pre-flight — and the default is to fail loudly, not to delete.** `CREATE UNIQUE INDEX IF NOT EXISTS` aborts the whole migration on the first duplicate row with no automatic remediation, and live writers deduping correctly doesn't help: partial replays, hand-edits, and operator-runs-without-checking all leave dupes. Pre-flight `SELECT` the conflicting keys and `RAISE EXCEPTION` naming them, so a human decides which row survives. **Corrected 2026-08-19** — this rule previously prescribed a blind `DELETE … WHERE rn > 1 ORDER BY id`, which discards production rows on an arbitrary survivor rule nobody chose. Auto-dedup only when the survivor rule is explicit and written into the migration comment, ordered by the column that encodes it (`updated_at DESC`), never by `id`. Migrations 0006 and 0008 predate this rule and carry no pre-flight at all; they applied cleanly, so prod has no duplicates, but a replay elsewhere would abort. Same defensive principle for `ALTER … NOT NULL` and `ALTER … CHECK`: backfill or remediate before the structural change. Master rule: **MIGRATION-PREFLIGHT**, `~/.claude/REVIEWER_CONVENTIONS.md` §6.
...
- **A continue/resume handoff that re-resolves its target by a time-windowed server guess silently orphans records once the window closes.** The Convos→Review loop carried only the person, then the server auto-linked the Review to that person's most recent open thread *with activity in the last 7 days* (`run-module.ts` step 8). Reviewing a conversation older than 7 days matched no thread, so the Review saved with `thread_id=null` — complete, with AI output, but invisible in Convos (the reads filter `thread_id IN <user's threads>`). Fix: carry the exact `threadId` through the handoff — the entry points already had it (`OpenConversations` used it as the React key; `ThreadReviewButton` lives on the thread page) and the schema + `input.threadId` already accepted it — and keep the 7-day window only as a fallback. Because the client now supplies the id, add the ownership check in the SAME change (`verifyThreadOwnership`, mirroring the existing person check at step 7) — never-trust-client-ids. Caught 2026-06-09; one orphaned row backfilled on prod. NB the symptom was reported as "AI retries failed then it didn't save," but the retries were a red herring — the row completed fine; verify the live DB row state before trusting the user's causal theory.
...
- **In the no-scroll FlowScreen, any control rendered BELOW a `fill` textarea gets clipped when the keyboard opens — and two stacked voice fields don't fit at all.** `useFlowViewport` shrinks the overlay to the visible band and `main` is `overflow-hidden`; a `VoiceInput`'s record button + "Recording…" indicator sit beneath its textarea, so an open keyboard pushes them past the clip boundary (the user can't tell recording is live). Two fixes: (1) blur the textarea when recording starts so the keyboard drops and the full band returns — `Redo` already did this for the same reason; (2) two stacked voice fields starve — the split-in-half `fill` layout collapses each textarea to ~0px with the keyboard up — so the two-column step uses fixed-height (`rows=3`) inputs in an `overflow-y-auto` column instead of the global no-scroll layout. Single-field `fill` flows are unchanged. Caught 2026-06-09 on the Review/Pulse two-column steps.
...
- **Any overlay/confirmation modal must use the shared `src/components/ui/confirm-dialog.tsx` (or match its contract), never a hand-rolled `fixed inset-0` div.** A bare backdrop div is invisible to AT and traps no focus: it needs `role="dialog"` + `aria-modal` + a labelled title (`aria-labelledby`)/description, a Tab/Shift-Tab focus trap, Escape-to-cancel, focus restore to the trigger on close, and a body-scroll lock. Two delete confirms had none until they were unified onto `ConfirmDialog`. New destructive confirms reuse it; if a modal genuinely can't, it still owes every item in that contract.
- **A global keyboard-focus ring must use `box-shadow`, not `outline` — and live in `globals.css` once, not per-component.** Inputs/buttons that set `focus:outline-none` (to swap the native ring for a border tint) leave keyboard/AT users with no visible focus (WCAG 2.4.7). The fix is a single `:focus-visible` rule in globals.css — but an element-level `:focus-visible` *outline* loses on specificity to the `focus:outline-none` utility (class+pseudo `(0,2,0)` > element+pseudo `(0,1,1)`) and gets suppressed. A `box-shadow` ring is uncontested (no utility sets it) and follows each element's own border-radius. Use `:focus-visible` (not `:focus`) so mouse clicks stay clean. Reference impl is in `globals.css`.
...
- **Webhook receivers go in `middleware.ts`'s public-route allowlist in the same change that adds the route.** The general rule is **GATE-EXCLUSIONS** (`~/.claude/REVIEWER_CONVENTIONS.md` §6). Pure-EQ specifics worth keeping: our matcher catches `/api/*` and 307-redirects cookie-less requests to `/login`, and Stripe does not follow redirects — so the symptom is silent, payment succeeds and nothing is credited. The post-ship smoke test must confirm the provider's delivery log shows 200; a green local build proves nothing. Caught on the 2026-05-30 Coins B2 first live test (fix `b1a46b0`).

### Secrets & key rotation

- **Rotating a key means updating it in TWO places: `.env.local` (local) and Vercel env (prod).** (Until 2026-06-11 it was THREE — GitHub Actions held the service-role/AI/admin keys for the CI E2E step. That step ran one login spec against PROD infra with the prod service-role key — all risk, near-zero coverage — so it was removed and every runtime key deleted from Actions; CI now holds only the two publishable `NEXT_PUBLIC_*` values for the build. E2E remains a local-only command behind `ALLOW_E2E_AGAINST_REMOTE=1`; the proper CI e2e rail against a dedicated test project is post-launch, `docs/implementation_plan_2026_06.md` item 1. If that rail ever lands, rotation grows a third place again — re-edit this rule then.) Historical context kept for the trap itself: Local `npm run build` / `npm test` / `tsc` all read `.env.local`, so they stay green after a rotation even when CI is broken — the green local checks prove nothing about CI. CI (`.github/workflows`) runs the E2E suite against live Supabase (`ALLOW_E2E_AGAINST_REMOTE: 1`); its first step is `createUser` via the service-role key, so a stale `SUPABASE_SECRET_KEY` in Actions fails every run with `createUser failed: Unregistered API key` — and a docs-only commit fails identically, which is the tell that it's a credential break, not code. List CI secrets + their last-set date with `gh secret list`; update with `gh secret set <NAME>` (paste when prompted — value never hits the transcript). Caught 2026-05-31: this session's post-leak rotation (Supabase service-role + Anthropic + OpenAI) updated `.env.local` + Vercel but not Actions, turning `main` CI red from `790698f` through B3 `9cb74d8` while every local check stayed green.
- **Only keys a CI step actually exercises belong in Actions secrets — lazily-instantiated keys don't.** `payments.ts`'s Stripe client is a lazy singleton (`new Stripe(...)` only on the request that needs it), so `next build` never touches it and no CI test makes a real Stripe call (the unit test sets fake `STRIPE_PRICE_*` inline). Result: Stripe keys live in `.env.local` + Vercel only, never in Actions, and CI is green without them. Before adding a secret to CI, confirm a CI step (build/test/E2E) genuinely reads it at run time — a build-time-unused, request-lazy key is dead weight in CI.
- **Vercel needs no GitHub secret — the two pipelines are independent.** Vercel deploys via its own Git integration watching the repo directly; GitHub Actions only typechecks/lints/tests/builds and never deploys. There is no Vercel token to add to Actions. Prod runtime env (incl. Stripe) lives in the Vercel dashboard; CI env lives in Actions secrets; they overlap only where a CI step needs the same value (Supabase/Anthropic/OpenAI/admin).

### Sentry / observability

- **`beforeSend` must scrub `event.exception.values[*].value`, not just `request.data` and `extra`.** SDK error messages are built from the **response** body, and response bodies carry data: Supabase `PostgrestError.message`/`details` returns the offending column values on conflict (`Key (user_id, name)=(…) already exists`) — verified. Sentry writes that into `exception.values[*].value`, which default scrub-spec tutorials never touch. Redact every `ex.value` in a shared `scrubEvent`, OR wrap at the capture site with a synthetic `new Error("short_tag")`.
  - **Corrected 2026-08-19:** this rule previously claimed Anthropic `APIError.message` "stringifies the full request body (echoes the prompt)". That is wrong on mechanism — `APIError.makeMessage()` in `@anthropic-ai/sdk@0.89.0` reads `error.message` from the parsed *response* body and never touches the request. Whether Anthropic's validation text ever quotes submitted content is unverified either way. **Keep the scrub regardless**: the PostgREST case alone makes it load-bearing, and a blanket `ex.value` redaction is correct for any provider whose error text you don't control. Recorded so nobody removes the scrub later on the grounds that "the Anthropic thing turned out to be false."
...
- **A user-displayed confidence/strength label is DERIVED server-side from verified evidence, never trusted from the model.** Observation `confidence` (`early`/`emerging`/`clear`) is computed by `deriveConfidence` from the count of DISTINCT verified entries (supporting − contradicting), AFTER `verifyQuotes` runs — the model emits a guess and the server overwrites it. The label can therefore never overstate the evidence the card actually shows. Same spirit as "server re-scores from raw inputs": any badge a user reads as a trust signal must be backed by something the server can verify, not the model's self-report. (Likely Playbook-worthy — pending cross-app validation.)
- **`counter_evidence[]` subtracts from confidence and is shown, not hidden.** Observations carry optional contradicting quotes; they're verified like supporting evidence, rendered under "But also", and each distinct contradicting entry lowers the net confidence. Asymmetric drop rule: a bad SUPPORTING quote drops the whole observation (grounding compromised); a bad COUNTER quote drops only itself. Don't "simplify" counter-evidence into a silent score adjustment — the visible exception is the coaching value.
- **The weekly focus loop is a TWO-WEEK cadence; `took_action` is server-counted, never model-judged.** Each reflection prescribes one `focus` (theme tied to a surfaced observation + concrete practice + 1–2 tools from prepare/before_you_send/triggered/review). The NEXT reflection grades it: `computeFocusActivity` counts real completed entries in those tools since the focus was set, and that count (≥1 = engaged) sets `focus_followup.took_action` — the model only writes the narrative `note`, which the server may override. `readPriorFocus` scans back several rows so a refusal week doesn't break the chain; no prior focus → `focus_followup` forced to `null` so a hallucinated look-back can't ship. The first reflection under any focus-shape version only SETS a focus (nothing to grade yet).
