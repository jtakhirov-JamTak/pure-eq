...
- **Any unauthenticated server-to-server receiver (Stripe/payment/email webhook) MUST be added to `middleware.ts`'s public-route allowlist in the same change that adds the route.** The matcher catches `/api/*` and 307-redirects every cookie-less request to `/login`; the provider does NOT follow redirects, so the handler never runs. Symptom is silent: payment succeeds, nothing is credited, the DB shows zero webhook events, and only the provider's delivery log reveals the 307. The webhook's signature IS its auth. The post-ship smoke test must confirm the provider's delivery log shows 200 — a green local build proves nothing here. Caught on the 2026-05-30 Coins B2 first live test (fix `b1a46b0`).
...
- **Symmetric `generator_version` check on both reader and writer.** `src/lib/insights/generate.ts` exports `GENERATOR_VERSION` (currently `"reflection_v6"`); the page reads it and gates rendering on `row.generator_version === GENERATOR_VERSION`, mismatch falls through to ReflectionKickoff (Playbook §16.17 generalized). Bump the constant whenever the `ai_json` shape OR semantics change — every edit so far (v3→v4 confidence relabel, v4→v5 focus loop, v5→v6 7-day window + single surfaced pattern) bumped it, which abandons old rows into a free regenerate rather than rendering them broken. NB v5→v6 was a SEMANTIC change (shape still parses) — bump anyway when an old row would mislead under the new UI promise.
