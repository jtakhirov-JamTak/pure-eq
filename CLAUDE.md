...
- **Sentry `beforeSend` must scrub `event.exception.values[*].value`, not just `request.data` and `extra`.** The canonical Sentry scrub spec ("delete request body, redact known extra keys, trim query strings") misses the primary PII channel: SDK error messages. Anthropic `APIError.message` stringifies the full response body on 4xx (which echoes the request prompt for validation failures). OpenAI does the same for Whisper. Supabase `PostgrestError.message`/`details` includes column values on conflict. Sentry writes those into `event.exception.values[*].value`, which the default-spec scrubber never touches. For any PII-sensitive app: redact every `ex.value` in a shared `scrubEvent`, OR wrap at the capture site with a synthetic `new Error("short_tag")` so the message is attacker-controlled-free. Our own tags carry the signal. Three of five independent reviewers caught this gap — the scrub spec that ships with every Sentry tutorial is insufficient for health/mental-health data.
- **Sentry default Breadcrumbs capture console args and fetch URLs — configure `beforeBreadcrumb` before enabling a DSN.** `Sentry.init(...)` without explicit integrations wires Breadcrumbs with `console: true` and fetch/xhr URL capture. A `console.error("thing failed", err)` stringifies the raw error (Supabase errors → column values in breadcrumbs, bypassing `beforeSend` entirely). Fetch breadcrumbs preserve `data.url` including query strings — user-typed search terms on `/api/persons?q=...` or similar leak through. Pattern: `beforeBreadcrumb` returns `null` for `category === "console"` and strips everything after `?` from `data.url` on fetch/xhr. This is as load-bearing as `beforeSend` for the health-data case.
- **`tracesSampleRate > 0` bypasses `beforeSend` — transactions use `beforeSendTransaction`.** A scrubber applied only to error events still leaks through traces: tracing events include full URL + query + route metadata and flow through the traces pipeline, not the error pipeline. If no tracing consumer exists (no dashboards, no alerts reading spans), ship `tracesSampleRate: 0`. "Collect now, use later" costs privacy budget for no benefit and can't be retroactively scrubbed. Same philosophy as "don't persist telemetry until a consumer reads it" — applies doubly to traces because they're enabled by the Sentry setup boilerplate.
- **Error-capture sites can themselves become flood vectors — latch captures in per-request fallback catches.** A `Sentry.captureException` inside `rate-limit.ts`'s Upstash fallback catch fires once per request. If Upstash credentials rotate in prod, every request to every rate-limited endpoint emits one event → thousands per minute on a busy instance. Worse than hiding the signal: the event quota exhausts so the alert that would have told you "Upstash is down" never fires. Pattern: module-level latch with a cooldown (`let lastCaptureAt = 0; if (Date.now() - last > 5 * 60 * 1000) capture()`) for any capture that runs in a code path hit on every request during an outage. Not needed for per-user paths (coach AI, transcribe) where natural request-rate limits the blast radius.
- **Auto-committed transcription needs a one-tap redo path when the text feeds an AI prompt.** Full confirmation banners add too much friction for mobile/walking use, but silent commit with no immediate undo lets speech-to-text errors flow straight into model input. The right pattern is: auto-commit, then allow redo of only the most recent voice commit for that specific mounted field instance.
- **Undo/redo buffers must invalidate on every parent-state mutation path, not just the flow that created them.** A voice commit writes a snapshot ref for Redo; any *other* deliberate user action that mutates the same controlled state (typing that clears a selection, tapping an X/clear button, picking a new item from a dropdown) must also drop the snapshot before the next rearm event. Otherwise Redo silently reverses the newer deliberate action — e.g. user voice-commits "Bob", taps X to clear, taps Redo expecting "re-record," and the component resurrects the cleared person. Audit checklist when adding a snapshot-restore undo: enumerate *every* handler inside the component that writes parent state (click handlers, input changes, clear buttons, suggestion-pick callbacks) and null the buffer in each. Plain typing that does NOT clear anything is the only path that legitimately keeps the buffer alive (per the "snapshot-restore after typing is correct" contract).
